 ...
 */
CQ c_queue; // 차량 생산 큐 전역 변수로 선언(생산자, 소비자 쓰레드가 공유하기 때문)
...
int main(int argc, char* argv[]) { // 메인 함수
	...
	*/
	...
	result_t = (end.tv_sec - start.tv_sec) + ((end.tv_usec - start.tv_usec)/1000000); // 수행 시간 체크
	
	printf("No Lock Experiment\n");
